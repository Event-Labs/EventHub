/**
 * Shared Frontend Utility for Refund Policy Structured Data & Text Generation
 */

/**
 * Validates list of refund rules
 * @param {Array<{days_before: number|string, refund_rate: number|string}>} rules
 * @returns {{ valid: boolean, error?: string, sortedRules?: Array<{days_before: number, refund_rate: number}> }}
 */
export function validateRefundRules(rules) {
  ...
  for (let idx = 0; idx < rules.length; idx++) {
    ...
    if (r.days_before === '' || r.days_before === null || r.days_before === undefined) {
      return { valid: false, error: `Mốc ${idx + 1}: Vui lòng nhập số ngày trước sự kiện.` }
    }
    if (r.refund_rate === '' || r.refund_rate === null || r.refund_rate === undefined) {
      return { valid: false, error: `Mốc ${idx + 1}: Vui lòng nhập tỷ lệ hoàn tiền.` }
    }
    ...
    if (!Number.isInteger(days) || days < 0) {
      return { valid: false, error: `Mốc ${idx + 1}: Số ngày trước sự kiện phải là số nguyên không âm (>= 0).` }
    }

    if (days > 365) {
      return { valid: false, error: `Mốc ${idx + 1}: Số ngày trước sự kiện không được vượt quá 365 ngày.` }
    }

    if (seenDays.has(days)) {
      return { valid: false, error: `Mốc ${idx + 1}: Số ngày trước sự kiện (${days} ngày) bị trùng lặp.` }
    }
    seenDays.add(days)

    if (!Number.isInteger(rate) || rate < 0 || rate > 100) {
      return { valid: false, error: `Mốc ${idx + 1}: Tỷ lệ hoàn tiền phải là số nguyên từ 0% đến 100%.` }
    }
 ...
 * @returns {string[]}
 */
export function generateRefundPolicyLines(policy) {
 ...
 * @returns {string}
 */
export function generateRefundPolicyText(policy) {
